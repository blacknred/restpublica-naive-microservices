...
router
    .post('/:pid/likes', ensureAuthenticated, likes,
        async (req, res, next) => {
            const newLike = {
    ...
    .get('/:pid/likes', ensureAuthenticated, likes, async (req, res, next) => {
    ...
    .delete('/:pid/likes', ensureAuthenticated, likes, async (req, res, next) => {
        try {
            const post = await Post.isExists(req.params.pid);
            if (!post) throw { status: 404, message: 'Post not found' };
