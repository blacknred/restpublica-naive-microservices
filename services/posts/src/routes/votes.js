...
router
    .post('/:pid/votes', ensureAuthenticated, votes,
        async (req, res, next) => {
            const newVote = {
    ...
    .get('/:pid/votes', ensureAuthenticated, votes, async (req, res, next) => {
    ...
    .delete('/:pid/votes/:oid', ensureAuthenticated, votes, async (req, res, next) => {
        try {
            const post = await Post.isExists(req.params.pid);
            if (!post) throw { status: 404, message: 'Post not found' };
