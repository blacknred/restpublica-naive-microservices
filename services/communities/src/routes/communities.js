...
router
    .post('/', ensureAuthenticated, communities, async (req, res, next) => {
        const newCommunity = {
    ...
    .put('/:cid', ensureAuthenticated, communities, async (req, res, next) => {
        try {
    ...
    .get('/', communities, async (req, res, next) => {
        ...
        try {
            if (list) data = await Community.getAllInList({ list, userId: req.user, limiter });
                ...
                switch (mode) {
                    ...
                    default: data = await Community.getAllByAdmin({
                        userId: admin, offset, reduced
                    });
                }
            } else if (profile) {
                switch (mode) {
